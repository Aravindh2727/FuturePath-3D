...
import { agricultureDepartments } from "../data/agricultureDepartments";
...
const catIcon: Record<string, ReactNode> = {
  "core-agriculture": <Sprout size={18} />,
  "horticulture-plant": <Leaf size={18} />,
  "animal-dairy": <Milk size={18} />,
  "fisheries-aqua": <Fish size={18} />,
  "forestry-environment": <Trees size={18} />,
  "agri-engineering": <Wrench size={18} />,
  agribusiness: <BarChart3 size={18} />,
  "food-processing": <ChefHat size={18} />,
  "specialized-applied": <Globe2 size={18} />,
  "diploma-short": <Settings size={18} />,
};

const catShade: Record<string, string> = {
  "core-agriculture": "from-emerald-500/20 to-green-400/10",
  "horticulture-plant": "from-lime-500/20 to-emerald-400/10",
  "animal-dairy": "from-amber-500/20 to-orange-400/10",
  "fisheries-aqua": "from-sky-500/20 to-blue-500/10",
  "forestry-environment": "from-teal-500/20 to-cyan-400/10",
  "agri-engineering": "from-indigo-500/20 to-slate-500/10",
  agribusiness: "from-violet-500/20 to-purple-400/10",
  "food-processing": "from-pink-500/20 to-rose-400/10",
  "specialized-applied": "from-emerald-500/20 to-green-500/10",
  "diploma-short": "from-slate-500/20 to-slate-300/10",
};
...
const pathTypeTone: Partial<Record<NonNullable<Department["pathType"]>, string>> = {
  ...
  management: "border-purple-300/50 bg-purple-400/15 text-purple-50",
  engineering: "border-blue-300/50 bg-blue-400/15 text-blue-50",
  "applied-farming": "border-lime-300/50 bg-lime-400/15 text-lime-50",
  "animal-science": "border-amber-300/50 bg-amber-400/15 text-amber-50",
  fisheries: "border-sky-300/50 bg-sky-400/15 text-sky-50",
  "food-processing": "border-rose-300/50 bg-rose-400/15 text-rose-50",
};
...
const AgricultureStreamPage = () => {
  ...
  const uniqueDepartments = useMemo(() => {
    ...
    agricultureDepartments.forEach((d) => {
  ...
  return (
    <div className="page-container py-12 space-y-8">
      <Seo
        title="Agriculture Courses After 12th | BSc Agri, Horticulture, Fisheries, Agribusiness"
        description="Find farming and agri-tech programs with subjects, eligibility, practical exposure, and career scope across agriculture and food industries."
        canonicalPath="/stream/agriculture"
      />
      <SectionHeader
        eyebrow="Agriculture Stream"
        title="Explore agriculture, agribusiness, and food systems"
        subtitle="Search, filter, and compare agriculture pathways—from core farming to agri-engineering and agribusiness."
      />
      ...
      <div className="glass-panel rounded-2xl border border-white/5 p-4 flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={() => navigate("/stream/agriculture")}
            className="text-xs px-3 rounded-full"
          >
            All categories
          </Button>
          <div className="flex flex-wrap gap-2">
            {agricultureCategories.map((cat) => (
              <Button
                key={cat.id}
                size="sm"
                ...
                onClick={() =>
                  activeCategory === cat.id ? navigate("/stream/agriculture") : navigate(`/stream/agriculture/category/${cat.id}`)
                }
              >
                {cat.title.split(" ")[0]}
              </Button>
        ...
        {agricultureCategories.map((cat) => (
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                variant={activeCategory === cat.id ? "primary" : "ghost"}
                onClick={() => navigate(`/stream/agriculture/category/${cat.id}`)}
                className="text-xs px-3"
              >
                {activeCategory === cat.id ? "Selected" : "Explore"}
              </Button>
              <Button variant="ghost" className="text-xs px-3" onClick={() => navigate(`/stream/agriculture/category/${cat.id}`)}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {filtered.length === 0 && (
          <div className="glass-panel rounded-xl border border-white/10 p-6 text-sm text-neutral-300">
            No agriculture paths match your filters. Try a different category or path type.
          </div>
        )}
      </div>
    </div>
...
export default AgricultureStreamPage;
